...
def categorical(df):
    ...
    """
    cols = df.columns
    num_col = num_cols = numerical(df)
    
    categorical_var = list(set(cols) - set(num_cols))
...
def convert(df,celsius):
    ...
    """
    converted_temp = df.copy()
    converted_temp[celsius] = converted_temp[celsius].apply(lambda x: x * (9.0/5.0) + 32.0)
...
weather_2012 = pd.read_csv(path)
print(weather_2012.head(2))
...
num_col = numerical(weather_2012)
print('num cols ', num_col)

cat_col = categorical(weather_2012)
...
dist = clear(weather_2012, 'Weather' ,weather_2012['Weather'])
print(dist['Cloudy'])
...
wind_speed_35_vis_25 = weather_2012[(weather_2012['Wind Spd (km/h)']> 35) & (weather_2012['Visibility (km)']==25)]
...
agg_values_ina_month(weather_2012,'Date/Time','Temp (C)', 'mean')
...
mean_weather = group_values(weather_2012,'Weather','mean')
...
df1 = convert(weather_2012,'Temp (C)')
